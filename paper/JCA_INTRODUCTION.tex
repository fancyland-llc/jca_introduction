...
the earlier CREM operator {[}3{]} from a single commutator to a
structured operator class. The JCA is defined on the coprime residue
lattice \((\mathbb{Z}/m)^\times\) for squarefree primorial \(m\) by two
independent axioms: \textbf{(i) bi-involutive equivariance} ---
\([A, P_\sigma] = 0\) and \(\{A, P_\tau\} = 0\) where
\(P_\sigma : r \mapsto -r \bmod m\) is additive reflection and
\(P_\tau : r \mapsto r^{-1} \bmod m\) is multiplicative inversion; and
\textbf{(ii) exact rank-4 invariant subspace} --- \(A\) admits a rank-4
invariant subspace \(V_4\) holding \(\geq 98\%\) of \(\|A\|_F^2\) at
every primorial. The CREM commutator \(C(m) = [D_\text{sym}, P_\tau]\)
is the first known member; this paper documents three more members and
demonstrates the algebra on a high-stakes empirical deployment: locating
the Kessler critical surface in the modern LEO debris environment.
